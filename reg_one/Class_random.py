...
if __name__ == '__main__':
    ...
    for j in range(1):
        for t in range(7):
            ...
            for i in range(10):
                torch.manual_seed(i)
                p = mp.Process(target=agent_random,args=(Xtrain,Ytrain[:,t].reshape(-1,1),Xtest,Ytest[:,t].reshape(-1,1),BaseNet,EvalNet,functions_,'lol',i,nb_batch,batch_size_sample,nb_ech,vogn_batch_size,batch_eval,nb_ep,ttt[t]))
                p.start()
                processes.append(p)
